              ...
              return (
                <div key={entry.id} className="mb-4 last:mb-0">
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    onClick={() => openWeightDetails(entry.id)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault()
                        openWeightDetails(entry.id)
                      }
                    }}
                    aria-label={`Open details for weight log on ${formatUKDate(entry.date)}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-2">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            setDeleteModal({ isOpen: true, id: entry.id })
                          }}
                          disabled={editingId === entry.id}
                          className="btn-card-icon-action"
