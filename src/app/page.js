...
export default function Home() {
  ...
  useEffect(() => {
    const loadTakenMedications = async () => {
      ...
      try {
        ...
        const glasses = hydrationData?.glasses ?? 0
        setTodayHydrationGlasses(glasses)
        if (glasses >= hydrationTarget && hydrationData?.updated_at) {
          setHydrationCompletedAt(hydrationData.updated_at)
        } else {
          setHydrationCompletedAt(null)
        }
      } catch (err) {
        console.error('Error fetching hydration:', err)
        setTodayHydrationGlasses(0)
        setHydrationCompletedAt(null)
      }
    ...
    const handleHydrationCompleted = () => {
      loadTakenMedications()
    }
    ...
    return (
      <div className="min-h-screen">
        ...
        {showAccountDeletedToast && (
          <div className="fixed top-24 right-4 z-50 rounded-lg shadow-lg border border-black/[0.08] dark:border-white/15 flex items-center gap-3 px-4 py-3 max-w-sm" style={{ backgroundColor: 'var(--bg-dropdown)' }}>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      {showToast && (
        <div className="fixed top-24 right-4 z-50 rounded-lg shadow-lg border border-black/[0.08] dark:border-white/15 flex items-center gap-3 px-4 py-3 max-w-sm" style={{ backgroundColor: 'var(--bg-dropdown)' }}>
          <div className="flex-shrink-0 flex items-center justify-center w-[1.125rem] h-[1.125rem] rounded-full bg-emerald-100 dark:bg-white">
            <svg className="w-3.5 h-3.5 text-emerald-600 dark:text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.4} d="M5 13l4 4L19 7" />
            </svg>
          </div>
      ...
      <div className="w-full sm:px-4 md:px-6 flex-1 flex flex-col min-h-0">
        ...
        <div className="my-6 order-[2] xl:order-none w-full min-w-0 max-w-full">
          ...
          <div className="block sm:hidden w-full min-w-0 [container-type:inline-size]">
            <div className="overflow-x-auto overscroll-x-contain scrollbar-hide snap-x snap-mandatory" style={{ WebkitOverflowScrolling: 'touch' }}>
        ...
        <div className="contents lg:grid grid-cols-1 lg:grid-cols-2 lg:items-stretch gap-6 mb-6 lg:order-5 xl:order-none">
          ...
          <div className="order-[8] lg:order-2 mb-6 lg:mb-0 lg:flex lg:flex-col lg:min-h-0 lg:h-full">
            ...
            <div className="card-inner p-4 sm:p-6 transition-all duration-300 ease-in-out">
              ...
              if (todayHydrationGlasses >= hydrationTarget && hydrationCompletedAt) {
                activities.push({
