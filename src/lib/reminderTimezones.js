...
export function parseUkLocalDateTime(dateStr, timeStr) {
  ...
  // Supabase may return `date` as YYYY-MM-DD or ISO datetime
  const normalizedDate = dateStr.split('T')[0].trim()
  ...
  return toDate(`${normalizedDate}T${hh}:${mm}:00`, { timeZone: APP_TIMEZONE })
}
